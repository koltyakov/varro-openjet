...
object ThemeBridge {
    ...
    private fun buildVariables(dark: Boolean, highContrast: Boolean): Map<String, String> {
        ...
        // VS Code's focusBorder also colors selected text and checkbox fills.
        // A Swing focus border can be too dark for those uses, even in a dark LAF.
        val focusBorder = readableAccent(
            listOfNotNull(
                uiColor("Component.focusColor"),
                uiColor("Component.focusedBorderColor"),
                linkColor,
                fallback(dark, 0x3794FF, 0x0066B8),
            ),
            listOf(sideBarBackground, inputBackground, widgetBackground),
        )
    ...
    internal fun css(color: Color): String =
        if (color.alpha == 255) {
            ...
            "rgba(${color.red}, ${color.green}, ${color.blue}, ${"%.3f".format(Locale.ROOT, color.alpha / 255.0)})"
        }

    internal fun readableAccent(candidates: List<Color>, backgrounds: List<Color>): Color {
        ...
        fun contrast(color: Color): Double = backgrounds.minOf { background ->
            val alpha = color.alpha / 255.0
            val visible = Color(
                (color.red * alpha + background.red * (1 - alpha)).toInt(),
                (color.green * alpha + background.green * (1 - alpha)).toInt(),
                (color.blue * alpha + background.blue * (1 - alpha)).toInt(),
            )
            val a = luminance(visible)
            val b = luminance(background)
            (maxOf(a, b) + 0.05) / (minOf(a, b) + 0.05)
        }
        return candidates.firstOrNull { contrast(it) >= 4.5 }
            ?: (candidates + listOf(Color.WHITE, Color.BLACK)).maxBy(::contrast)
    }
