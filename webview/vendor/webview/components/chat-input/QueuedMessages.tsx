      ...
      <Show when={(props.pendingSteers?.length ?? 0) > 0}>
        <div class="chat-queue-container chat-steer-container">
          <div
            ...
            role="list"
            aria-label="Steered messages"
            ref={(element) =>
              bindQueueOverflowFade(element, () => props.pendingSteers?.length ?? 0)
            }
          >
            <For each={props.pendingSteers}>
              {(item) => (
                <div class="chat-queue-item" role="listitem" data-steered-message-id={item.id}>
                  <div class="chat-queue-body">
                    <span class="chat-steer-icon" aria-hidden="true">
                      <UiIcon source={hourglassIcon} width={12} height={12} />
                    </span>
                    <span class="chat-queue-label" title={item.text}>
                  ...
                  >
                    <div class="chat-queue-body">
                      ...
                      <span
                        ...
                        onMouseEnter={() => {
                          setLabelTruncated(
                            !!labelRef && labelRef.scrollWidth > labelRef.clientWidth
                          );
                        }}
                      >
                        {label}
                    ...
                    <div class="chat-queue-actions">
                      ...
                      <button
                        ...
                        aria-label={
                          isEditing() ? 'Cancel queued message edit' : 'Edit queued message'
                        }
                      >
                        <Show
                          when={isEditing()}
