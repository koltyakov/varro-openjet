...
it('restores IDE preferences and mirrors writes and removals for the next load', () => {
  ...
  expect(Object.hasOwn(host, 'localStorage')).toBe(false);
  ...
  expect(send).toHaveBeenLastCalledWith({
    type: 'host/storage',
    payload: { key: 'draft', value: 'text' },
  });
  storage.removeItem('preference');
  expect(send).toHaveBeenLastCalledWith({
    type: 'host/storage',
    payload: { key: 'preference', value: null },
  });
...
it('updates sibling views without echoing writes and isolates another project', () => {
  ...
  host.dispatchEvent(
    new MessageEvent('message', {
      data: { type: 'host/storage', payload: { key: 'auto-context', value: 'false' } },
    })
  );
  ...
  host.dispatchEvent(
    new MessageEvent('message', {
      data: { type: 'host/storage', payload: { key: 'auto-context', value: null } },
    })
  );
