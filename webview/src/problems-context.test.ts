...
import { formatEditorProblems, getProblemsSeverity, parseIssueAttachment } from '../vendor/webview/lib/editor-problems';
import { getProblemCompletionItems } from '../vendor/webview/components/chat-input/completion';
import { cloneInlineProblems, deduplicateInlineProblems, formatInlineProblem, parseInlineProblem, problemReferenceMarker } from '../vendor/webview/lib/editor-problems';

const diagnostic = {
  path: '/project/app.kt', severity: 'error' as const, message: 'Unresolved reference\nDetails',
  line: 2, column: 4, endLine: 2, endColumn: 8, code: 'Kotlin', intersectsSelection: true,
};

describe('JetBrains problems context', () => {
  it('accepts the settings opt out in live configuration updates', () => {
    const message = { type: 'config/update', payload: {
      enableProblemsContext: false, desktopSessionPaneSide: 'right', defaultPermissionMode: 'auto',
      chatFontSize: 13, chatEditorFontSize: 12, chatFontFamily: 'default',
    } };
  ...
  it('round-trips grouped problem chips and preserves detached snapshots', () => {
    const reference = { id: 'jetbrains-1', diagnostic: { ...diagnostic }, group: [{ ...diagnostic }] };
    ...
    expect(deduplicateInlineProblems(text, [copy], [diagnostic])).toEqual({ text: 'Fix ', references: [] });
  });

  it('accepts native snapshots and formats selected details with JetBrains branding', () => {
    const payload = {
      workspacePath: '/project', activeFile: { path: diagnostic.path, relativePath: 'app.kt', language: 'kotlin' },
      selection: null, diagnostics: [diagnostic], diagnosticsTotal: 30,
      diagnosticCounts: { errors: 29, warnings: 1 },
    };
    expect(parseExtensionMessage({ type: 'context/update', payload })).not.toBeNull();
    const text = formatEditorProblems(payload)!;
  ...
  it('accepts captured Problems actions and rejects invalid coordinates', () => {
    ...
    expect(parseExtensionMessage(message)).toEqual(message);
    expect(parseExtensionMessage({ ...message, payload: { diagnostics: [{ ...diagnostic, column: 0 }] } })).toBeNull();
  });

  it('searches native workspace problems', () => {
    const items = getProblemCompletionItems({ diagnostics: [diagnostic], total: 1 }, 'unresolved', '/project');
    expect(items.some(item => item.type === 'problems' && item.diagnostic?.path === diagnostic.path)).toBe(true);
  });
});
