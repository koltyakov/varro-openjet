...
const context: DatabaseContext = {
  name: 'Demo SQLite', dataSource: 'Demo SQLite', dialect: 'SQLite', filter: '',
  scope: 'datasource', origin: 'workspace', columns: [], rows: [], selectedRowCount: 0,
  pendingChanges: false, cellEditing: false, pageStart: 0, truncated: false,
  dataSources: [{ id: 'demo', name: 'Demo SQLite', dialect: 'SQLite', connected: true, catalogs: [], schemas: ['main'] }],
};
const part = (text: string): TextPart => ({ type: 'text', id: 'part', sessionID: 'session', messageID: 'message', text });

describe('database context in Up/Down prompt history', () => {
  it('recalls only the prompt when automatic context is a separate part', () => {
    expect(getUserMessageHistoryText([part("What's the current DB?"), part(formatDatabaseContext(context))]))
  ...
  it('handles old grid snapshots and longer fences needed by SQL containing backticks', () => {
    const grid: DatabaseContext = { ...context, scope: 'table', origin: 'grid', ddl: 'select ```literal```;' };
    const formatted = formatDatabaseContext(grid);
    expect(formatted).toContain('````json');
    expect(getUserMessageHistoryText([part(`Fix this query.\r\n\r\n${formatted.replaceAll('\n', '\r\n')}`)]))
      .toBe('Fix this query.');
